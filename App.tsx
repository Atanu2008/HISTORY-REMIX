...
const HISTORICAL_EVENTS = ['Ancient Rome', 'Viking Age', 'Renaissance Italy', 'Wild West', 'Roaring Twenties', 'Moon Landing'];
...
function App() {
    ...
    const handleGenerateClick = async () => {
        ...
        HISTORICAL_EVENTS.forEach(event => {
            initialImages[event] = { status: 'pending' };
        });
        setGeneratedImages(initialImages);

        const concurrencyLimit = 2; // Process two events at a time
        const eventsQueue = [...HISTORICAL_EVENTS];

        const processEvent = async (event: string) => {
            try {
                const resultUrl = await generateHistoricalImage(uploadedImage, event);
                setGeneratedImages(prev => ({
                    ...prev,
                    [event]: { status: 'done', url: resultUrl },
                }));
            } catch (err) {
                const errorMessage = err instanceof Error ? err.message : "An unknown error occurred.";
                setGeneratedImages(prev => ({
                    ...prev,
                    [event]: { status: 'error', error: errorMessage },
                }));
                console.error(`Failed to generate image for ${event}:`, err);
            }
        };
    ...
    const handleRegenerateEvent = async (event: string) => {
        if (!uploadedImage) return;

        // Prevent re-triggering if a generation is already in progress
        if (generatedImages[event]?.status === 'pending') {
            return;
        }
        
        console.log(`Regenerating image for ${event}...`);

        // Set the specific event to 'pending' to show the loading spinner
        setGeneratedImages(prev => ({
            ...prev,
            [event]: { status: 'pending' },
        }));

        // Call the generation service for the specific event
        try {
            const resultUrl = await generateHistoricalImage(uploadedImage, event);
            setGeneratedImages(prev => ({
                ...prev,
            ...
            console.error(`Failed to regenerate image for ${event}:`, err);
        }
    };
    ...
    const handleDownloadIndividualImage = (event: string) => {
        const image = generatedImages[event];
        if (image?.status === 'done' && image.url) {
            const link = document.createElement('a');
            link.href = image.url;
            link.download = `history-remix-${event}.jpg`;
    ...
    const handleDownloadAlbum = async () => {
        ...
        try {
            // FIX: Explicitly cast the result of Object.entries to fix type inference issues.
            // This ensures 'image' is correctly typed as GeneratedImage within filter and reduce.
            // Also corrected a potential typo from `image!.url!` to `image.url!`.
            const imageData = (Object.entries(generatedImages) as [string, GeneratedImage][])
                .filter(([, image]) => image.status === 'done' && image.url)
                .reduce((acc, [event, image]) => {
                    acc[event] = image.url!;
                    return acc;
                }, {} as Record<string, string>);

            if (Object.keys(imageData).length < HISTORICAL_EVENTS.length) {
            ...
            link.download = 'history-remix-album.jpg';
            document.body.appendChild(link);
            link.click();
            document.body.removeChild(link);
        ...
        <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 pb-24 overflow-hidden relative">
            ...
            <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1 min-h-0">
                <div className="text-center mb-10">
                    <h1 className="text-6xl md:text-8xl font-caveat font-bold text-neutral-100">History Remix</h1>
                    <p className="font-permanent-marker text-neutral-300 mt-2 text-xl tracking-wide">Place yourself in famous historical events.</p>
                </div>
                            ...
                            <div className="w-full max-w-sm flex-1 overflow-y-auto mt-4 space-y-8 p-4">
                                {HISTORICAL_EVENTS.map((event) => (
                                    <div key={event} className="flex justify-center">
                                         <PolaroidCard
                                            caption={event}
                                            status={generatedImages[event]?.status || 'pending'}
                                            imageUrl={generatedImages[event]?.url}
                                            error={generatedImages[event]?.error}
                                            onShake={handleRegenerateEvent}
